...
export class TasksService {
  ...
  updateTodoStatus(id: string, status: 'pending' | 'completed') {
    ...
    return fromFetch(
      `https://firestore.googleapis.com/v1beta1/projects/todo-ab144/databases/(default)/documents/todos/${id}?updateMask.fieldPaths=status`,
      {
        method: 'PATCH',
  ...
  deleteTodo(id: string) {
    return fromFetch(
      `https://firestore.googleapis.com/v1beta1/projects/todo-ab144/databases/(default)/documents/todos/${id}`,
      {
        method: 'DELETE',
      }
    ).pipe(
